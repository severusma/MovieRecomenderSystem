...
public class Normalize {
    ...
    public static class NormalizeReducer extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context)
            ...
            int denominator = 0;
            Map<String, Integer> movieRelationMap = new HashMap<>();
            for (Text value : values) {
                String[] movieBRelation = value.toString().trim().split("=");
                int relation  = Integer.parseInt(movieBRelation[1]);
                movieRelationMap.put(movieBRelation[0], relation);
                denominator += relation;
            }
            Iterator iterator = movieRelationMap.entrySet().iterator();
